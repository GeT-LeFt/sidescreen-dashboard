...
下面是本地正在运行的 `/wide` 界面，使用独立浏览器视口截取。为了完整展示界面，**会话、额度、设备名称、日程、通知、媒体与应用列表均使用虚构示例数据**，图片右上角标有 DEMO。Codex 周额度、进度条和任务状态完整保留；示例数字不代表实际账号配额。系统性能卡保留截图时的本机读数，顶部封面为示意图。

### 信息总览 · 把主屏还给工作

![宽屏性能看板：Codex 周额度、媒体与日程使用示例数据](docs/images/dashboard.png)

### 任务视图 · 运行与完成分开看

![AI 任务页：Windows 与 Mac 上运行中和已完成任务的示例](docs/images/sessions.png)

### 操控台 · 常用操作触手可及

![声音与应用控制页：示例应用启动器和分应用音量](docs/images/controls.png)

### 媒体页 · 给音乐留一整页

![媒体页：示例曲目、多行歌词与播放控制](docs/images/media.png)
